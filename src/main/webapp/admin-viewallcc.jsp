                        ...
                        <table id="example3" class="display text-nowrap" style="min-width: 845px" >
                          ...
                          <tbody>
                           <c:forEach items="${cclist}" var="c">
                            <tr>
                              <td><strong><c:out value="${c.id}"/></strong></td>
                              <td><c:out value="${c.name}"/></td>
                              <td><c:out value="${c.gender}"/></td>
                              <td>
                                <c:out value="${c.email}"/></a
                                >
                              </td>
                              <td>
								<c:out value="${c.qualification}"/>
                              </td>
                              
             				  <td class="status ${c.status == 'Registered' ? 'registered' : (i.status == 'Approve' ? 'Approved' : 'Rejected')}">
                              		<a href="javascript:void(0);">
                               		  <strong><c:out value="${c.status}"/></strong>
                              		 </a>
                               </td>
                             
                              <td style="white-space: nowrap;">
                              &nbsp;&nbsp;
                   				<a class="action-btn accept-btn" href='<c:url value="updateccstatus?id=${c.id}&status=Approved"></c:url>'>
								   <span class="btn-icon">&#10004;</span> 
								</a>&nbsp;&nbsp;&nbsp;
								<a class="action-btn reject-btn" href='<c:url value="updateccstatus?id=${c.id}&status=Rejected"></c:url>'>
								   <span class="btn-icon">&#10006;</span> 
								</a>
							  </td>
                              ...
                              <td>
                                ...
                                ></a>
                                <a class="btn btn-xs sharp btn-danger" onclick="confirmDelete('<c:url value='deletecc?id=${c.id}'/>')"><i class="fa fa-trash"></i></a>
                              </td>
                            </tr>
                            </c:forEach>
